...
\section{模型架构}\label{model-architecture}
...
\subsection{Hybrid Attention}\label{hybrid-attention}
...
\subsubsection{Kimi Delta Attention}\label{kimi-delta-attention}
...
\subsubsection{Gated MLA}\label{gated-mla}
...
\subsection{Attention Residuals}\label{attention-residuals}
...
\subsection{Stable LatentMoE}\label{stable-latentmoe}
...
\subsubsection{Normalized LatentMoE}\label{normalized-latentmoe}
...
\subsubsection{Sigmoid Tanh Unit GLU}\label{sigmoid-tanh-unit-glu}
...
\subsubsection{Quantile Balancing}\label{quantile-balancing}
...
\subsection{原生视觉}\label{native-vision}
...
\subsection{Per-Head Muon}\label{per-head-muon}
...
\subsection{预训练数据}\label{pre-training-data}
...
\subsection{缩放定律}\label{scaling-law}
...
\subsection{训练配方}\label{training-recipe}
...
\subsection{长上下文扩展}\label{long-context-extension}
...
\subsection{主要结果}\label{main-results}

\subsubsection{基准测试}\label{benchmarks}
...
\subsubsection{基线模型}\label{baselines}
...
\subsubsection{评测配置}\label{evaluation-configurations}
...
\subsubsection{结果}\label{results}
...
\subsection{内部评测}\label{internal-evaluation}

\subsubsection{能力评测}\label{capability-evaluation}
...
\subsubsection{网络安全评测}\label{cyber-security-evaluation}
...
\subsection{第三方评测}\label{third-party-evaluation}
...
\subsection{成本效率}\label{cost-efficiency}
...
\section{Sigmoid Tanh Unit GLU 细节}\label{b-details-of-sigmoid-tanh-unit-glu}
...
\section{分位数均衡的推导}\label{c-derivation-of-quantile-balancing}
...
\section{基于直方图的分位数估计}\label{d-histogram-based-quantile-estimation}
...
\section{MoonEP 通用上界证明}\label{e-moonep-general-upper-bound-proof}
...
\section{聊天模板}\label{f-chat-template}
